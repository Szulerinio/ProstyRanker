...
const getCriteria = (input) => {
    const criteria = input
        ...
        const plotPoints = plotPts.map((point) => {
            ...
            const values = pointXY.split(",").map((value) => +value);
            return values;
        });
...
const getYValue = (xValue, criterium) => {
    for (let i = 1; i < criterium.plotPoints.length; i++) {
        if (xValue <= criterium.plotPoints[i][0]) {
            const xDistance = criterium.plotPoints[i][0] - criterium.plotPoints[i - 1][0];
            const yDistance = criterium.plotPoints[i][1] - criterium.plotPoints[i - 1][1];
...
const main = (input) => {
    ...
    const ranking = matrix.map((row, index) => {
        const value = row.values
            .map((xValue, index) => {
            return ((getYValue(xValue, criteria[index]) || 0) * criteria[index].weight);
        })
            .reduce((prev, current) => {
